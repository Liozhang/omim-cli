...
# Default database path. Resolution order:
#   1. OMIM_DB / OMIM_DBFILE env var (user-configurable default)
#   2. packaged omim/data/omim.sqlite3 (if shipped)
#   3. ~/omim_data/omim.sqlite3 (per-user build location)
DEFAULT_DB = (os.environ.get('OMIM_DB') or os.environ.get('OMIM_DBFILE')
              or os.path.join(BASE_DIR, 'data', 'omim.sqlite3'))

if not os.environ.get('OMIM_DB') and not os.environ.get('OMIM_DBFILE') \
        and not os.path.isfile(DEFAULT_DB):
    DEFAULT_DB = os.path.join(HOME, 'omim_data', 'omim.sqlite3')

DEFAULT_URL = 'https://omim.org'
